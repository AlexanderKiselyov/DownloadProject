namespace BelkaCloudDownloader.Gui
{
	...
	/// <summary>
	/// Protocol for downloading Dropbox information and documents.
	/// </summary>
	internal sealed class DropboxProtocol : Protocol
	{
		public override IList<string> AuthenticationMethods { get; } = new List<string>
		{
			"Dropbox consent screen"
		};
		...
		public override Control AuthenticationControl(string authenticationMethod)
		{
			...
			{
				case "Dropbox consent screen":
					{
						return new ConsentScreenControl(
						   token => DropboxDownloader.Authenticate(token, this.Log));
					}
			}
		...
		public override async Task<object> Execute(
			...
			ProgressBar currentOperationProgressBar)
		{
			var downloader = new DropboxDownloader(cancellationToken);
			...
			{
				throw new InternalErrorException("Incorrect authentication control for Dropbox protocol.");
			}
			var token = consentScreenControl.RefreshToken;
			var settings = new DriveDownloaderSettings
			{
				RefreshToken = token,
				TargetDirectory = targetDirectory + Path.DirectorySeparatorChar + "Dropbox",
				DoDownloadFiles = selectedCapabilities.Contains("Files from Dropbox")
			};

			return await downloader.Download(settings, cancellationToken, currentOperationProgressBar);
		}
	}
}
